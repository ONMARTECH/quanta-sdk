...
def main():
    ...
    print("[2/3] Writing unique uncompressible quantum phase data across 16.0 GiB...")
    ...
    used_gb = vm_total.used / (1024**3)
    total_gb = vm_total.total / (1024**3)
    print(f"[+] Secondary buffer populated in {t_fill2:.2f}s")
    print("\n" + "*" * 75)
    print(f"🚀 TRUE UNCOMPRESSED PHYSICAL RESIDENT MEMORY (RSS): {rss_total:.2f} GB!")
    print(f"   Total System RAM Used: {used_gb:.2f} GB / {total_gb:.2f} GB ({vm_total.percent}%)")
    print("*" * 75)
