...
Let an episodic engram statevector $|\psi_0\rangle \in \mathcal{H}$ be registered in the hippocampal buffer at time $t = 0$. Under thermal and ionic fluctuations, its density operator $\rho(t)$ evolves via the Lindblad dephasing generator:
\begin{equation}
    \frac{d\rho}{dt} = -i [H_{\text{free}}, \rho] + \sum_{k=1}^d \gamma_k \left( L_k \rho L_k^\dagger - \frac{1}{2} \{ L_k^\dagger L_k, \rho \} \right)
\end{equation}
where $L_k = |k\rangle\langle k|$ are local dephasing operators with effective rate damped by dopaminergic synaptic capture (Frey \& Morris, 1997):
\begin{equation}
    \gamma_k = \frac{\gamma_0}{1 + \lambda_D \cdot D_k}
\end{equation}
During offline sleep, Sharp-Wave Ripples probabilistically sample degraded engrams according to Boltzmann salience weights $P_{\text{replay}}(j) \propto \exp(D_j / T)$. Under the REM sleep orthogonalization potential $\mathcal{L}_{\text{REM}}$, even when the replayed engrams have degraded in fidelity ($\mathcal{F} \approx 0.85 - 0.94$), gradient descent preserves asymptotic convergence to mutually orthogonal task subspaces:
\begin{equation}
    \lim_{c \to \infty} \mathcal{L}_{\text{REM}}^{(c)} \le \epsilon \ll \mathcal{L}_{\text{wake}}
\end{equation}
proving that the neocortical sleep consolidation mechanism is intrinsically robust against non-ideal biological noise.
\end{theorem}
