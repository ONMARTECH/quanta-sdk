...
class MLXSimulator(StateVectorSimulator):
    ...
    def apply(
        ...
        # Reorder tensor axes back to [0, 1, ..., n-1]
    ...
    def apply_phase(self, index: int, phase: complex) -> None:
        ...
        """
        mx = self._mx
        flat = np.array(mx.reshape(self._state, (-1,)), dtype=np.complex64)
        flat[index] *= phase
        self._state = mx.array(flat.reshape([2] * self.num_qubits))  # type: ignore[assignment]
        mx.eval(self._state)
